 ...
 *
 *
 *To develop a symbol-table implementation that is feasible for use with clients like Lookup and Index, 
 *we need the flexibility of linked lists and the efficiency of binary search. Binary search trees,
 * which we consider next, provide just this combination.
 * 
 * BST
-a binary tree in which for each node, 
	-value of all nodes in left subtree is less than the node, 
	-value of all nodes in right subtree is greater
	-(recursive structure) and this must be true for all nodes (Except leaf nodes) so even the left and right subtrees are BSTs. 	
	
	performance
	-best case (perfectly balanced)- cost of every put operation/get request is proportional to lg N or less
	-average case- 2 ln N for a random put/get
	-worst case- each node has only 1 child (perfectly UNBALANCED-occurs when we insert the keys in order)- exactly the
	 same behaviour as a linked list - linear time
 *************************************************************************/
import bagsStacksQueues.stackLinkedList;
...
public class BST<Key extends Comparable<Key>, Val> implements Iterable<Key> {
    ...
    private class Node {
        ...
        //The key type must be Comparable, but the type of the value is not specified, so a BST node can hold any kind of data
    ...
    ***********************************************************************/
    ...
    private Node insert(Node x, Key key, Val value) {
        ...
        int cmp = key.compareTo(x.key);//use compareTo because our values could be strings, Integers etc.
        if      (cmp < 0) x.left  = insert(x.left,  key, value);//if smaller put in left subtree
        else if (cmp > 0) x.right = insert(x.right, key, value);//if bigger put in right subtree
        else x.val = value;
        return x;
    }
    ...
    ***********************************************************************/
    ...
    public Val get(Key key) {
        Node x = root;
        while (x != null) {//binary search - if less search left subtree, if greater search right subtree
    ...
    ***********************************************************************/
    ...
    private class Inorder implements Iterator<Key> {
        private stackLinkedList<Node> stack = new stackLinkedList<Node>();
    ...
    public static void main(String[] args) {
        ...
        StdOut.println(st.get("www.cnn.com"));
        StdOut.println();
